...
void make_language_map(LanguageVersions& map, pugi::xml_node& doc, const char* tag)
{
    for (pugi::xml_node node: doc.children(tag)) {
        std::string lang = node.attribute("lang").value();
        std::string value = node.text().as_string();
        if (value.empty() && lang.empty()) {
            continue;
        }
        map.insert(std::pair<std::string, std::string>(lang, value));
    }
}
...
parse_acceptor(pugi::xml_node& node) {
    ...
    make_language_map(acceptor.title_, node, "title");
    make_language_map(acceptor.description_, node, "description");
    
    return std::pair<std::string, ZHfstOspellerAcceptorMetadata>(descr, acceptor);
}
...
parse_errmodel(pugi::xml_node& node) {
    ...
    make_language_map(errmodel.title_, node, "title"); 
    make_language_map(errmodel.description_, node, "description");
...
ZHfstOspellerXmlMetadata::deserialize(pugi::xml_document& doc)
{
    ...
    if (info.locale_.empty()) {
        throw ZHfstMetaDataParsingError("Missing locale");
    }
    make_language_map(info.title_, info_node, "title");
    if (info.title_.size() == 0) {
        throw ZHfstMetaDataParsingError("Missing titles");
    }
    make_language_map(info.description_, info_node, "description");
    if (info.description_.size() == 0) {
        throw ZHfstMetaDataParsingError("Missing descriptions");
    }
